...
def main_mps(args):
    ...
    cfg.mps_device = torch.device("mps")
    ...
    my_model = my_model.to(cfg.mps_device)
    ...
    while epoch < max_num_epochs:
        ...
        for i_iter, (imgs, img_metas, train_vox_label, train_grid, train_pt_labs) in enumerate(train_dataset_loader):
            ...
            imgs = imgs.to(cfg.mps_device)
            train_grid = train_grid.to(torch.float32).to(cfg.mps_device)
            if cfg.lovasz_input == 'voxel' or cfg.ce_input == 'voxel':
                voxel_label = train_vox_label.type(torch.LongTensor).to(cfg.mps_device)
            if cfg.lovasz_input == 'points' or cfg.ce_input == 'points':
                train_pt_labs = train_pt_labs.to(cfg.mps_device)
        ...
        with torch.no_grad():
            for i_iter_val, (imgs, img_metas, val_vox_label, val_grid, val_pt_labs) in enumerate(val_dataset_loader):
                ...
                imgs = imgs.to(cfg.mps_device)
                val_grid_float = val_grid.to(torch.float32).to(cfg.mps_device)
                val_grid_int = val_grid.to(torch.long).to(cfg.mps_device)
                vox_label = val_vox_label.to(cfg.mps_device)
                val_pt_labs = val_pt_labs.to(cfg.mps_device)
...
if __name__ == '__main__':
    ...
    from torch.backends import mps as BACKEND
    main_mps(args)
